...
public class RepairJob extends AbstractFuture<RepairResult> implements Runnable
{
    ...
    public void run()
    {
        ...
        List<VirtualEndpoint> allEndpoints = new ArrayList<>(session.endpoints);
        ...
        {
            ListenableFuture<List<VirtualEndpoint>> allSnapshotTasks;
            if (isIncremental)
            {
            ...
            else
            {
                ...
                List<ListenableFuture<VirtualEndpoint>> snapshotTasks = new ArrayList<>(allEndpoints.size());
            ...
            validations = Futures.transformAsync(allSnapshotTasks, new AsyncFunction<List<VirtualEndpoint>, List<TreeResponse>>()
            {
                public ListenableFuture<List<TreeResponse>> apply(List<VirtualEndpoint> endpoints)
                {
    ...
    private AsyncFunction<List<TreeResponse>, List<SyncStat>> optimisedSyncing()
    {
        return trees ->
        {
            ...
            ImmutableMap<VirtualEndpoint, HostDifferences> reducedDifferences = ReduceHelper.reduce(diffHolder, preferSameDCFilter);

            for (int i = 0; i < trees.size(); i++)
            {
                VirtualEndpoint address = trees.get(i).endpoint;
                ...
                if (streamsFor != null)
                {
                    ...
                    for (VirtualEndpoint fetchFrom : streamsFor.hosts())
                    {
    ...
    private String getDC(VirtualEndpoint address)
    {
     ...
     */
    private ListenableFuture<List<TreeResponse>> sendValidationRequest(Collection<VirtualEndpoint> endpoints)
    {
     ...
     */
    private ListenableFuture<List<TreeResponse>> sendSequentialValidationRequest(Collection<VirtualEndpoint> endpoints)
    {
        String message = String.format("Requesting merkle trees for %s (to %s)", desc.columnFamily, endpoints);
        logger.info("{} {}", previewKind.logPrefix(desc.sessionId), message);
        Tracing.traceRepair(message);
        ...
        Queue<VirtualEndpoint> requests = new LinkedList<>(endpoints);
        VirtualEndpoint address = requests.poll();
        ...
        {
            final VirtualEndpoint nextAddress = requests.poll();
     ...
     */
    private ListenableFuture<List<TreeResponse>> sendDCAwareValidationRequest(Collection<VirtualEndpoint> endpoints)
    {
        String message = String.format("Requesting merkle trees for %s (to %s)", desc.columnFamily, endpoints);
        logger.info("{} {}", previewKind.logPrefix(desc.sessionId), message);
        Tracing.traceRepair(message);
        ...
        Map<String, Queue<VirtualEndpoint>> requestsByDatacenter = new HashMap<>();
        for (VirtualEndpoint endpoint : endpoints)
        {
            String dc = DatabaseDescriptor.getEndpointSnitch().getDatacenter(endpoint);
            Queue<VirtualEndpoint> queue = requestsByDatacenter.get(dc);
            if (queue == null)
            {
        ...
        for (Map.Entry<String, Queue<VirtualEndpoint>> entry : requestsByDatacenter.entrySet())
        {
            Queue<VirtualEndpoint> requests = entry.getValue();
            VirtualEndpoint address = requests.poll();
            ValidationTask firstTask = new ValidationTask(desc, address, nowInSec, previewKind);
            logger.info("Validating {}", address);
            session.waitForValidation(Pair.create(desc, address), firstTask);
