...
public class RepairRunnable extends WrappedRunnable implements ProgressEventNotifier
{
    ...
    {
        public final Set<VirtualEndpoint> endpoints;
        public final Collection<Range<Token>> ranges;

        public CommonRange(Set<VirtualEndpoint> endpoints, Collection<Range<Token>> ranges)
        {
    ...
    protected void runMayThrow() throws Exception
    {
        ...
        Set<VirtualEndpoint> allNeighbors = new HashSet<>();
        ...
        try
        {
            ...
            {
                Set<VirtualEndpoint> neighbors = ActiveRepairService.getNeighbors(keyspace, keyspaceLocalRanges, range,
        ...
        {
            Set<VirtualEndpoint> actualNeighbors = Sets.newHashSet(Iterables.filter(allNeighbors, FailureDetector.instance::isAlive));
     ...
     */
    @VisibleForTesting
    static List<CommonRange> filterCommonRanges(List<CommonRange> commonRanges, Set<VirtualEndpoint> liveEndpoints, boolean force)
    {
        if (!force)
        {
            return commonRanges;
        ...
        else
        {
            ...
            {
                Set<VirtualEndpoint> endpoints = ImmutableSet.copyOf(Iterables.filter(commonRange.endpoints, liveEndpoints::contains));
    ...
    private void addRangeToNeighbors(List<CommonRange> neighborRangeList, Range<Token> range, Set<VirtualEndpoint> neighbors)
    {
    ...
    private Thread createQueryThread(final int cmd, final UUID sessionId)
    {
        return NamedThreadFactory.createThread(new WrappedRunnable()
        {
            ...
            public void runMayThrow() throws Exception
            {
                ...
                VirtualEndpoint source = FBUtilities.getBroadcastAddressAndPort();
                ...
                while ((status = state.waitActivity(timeout)) != TraceState.Status.STOPPED)
                {
                    ...
                    for (UntypedResultSet.Row r : result)
                    {
                        ...
                        VirtualEndpoint eventNode = VirtualEndpoint.getByAddressOverrideDefaults(r.getInetAddress("source"), port);
