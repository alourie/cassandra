...
public class HintedHandoffMetrics
{
    ...
    private final LoadingCache<VirtualEndpoint, DifferencingCounter> notStored = Caffeine.newBuilder()
                                                                                            .executor(MoreExecutors.directExecutor())
                                                                                            .build(DifferencingCounter::new);

    /** Total number of hints that have been created, This is not a cache. */
    private final LoadingCache<VirtualEndpoint, Counter> createdHintCounts = Caffeine.newBuilder()
                                                                                        .executor(MoreExecutors.directExecutor())
                                                                                        .build(address -> Metrics.counter(factory.createMetricName("Hints_created-" + address.toString().replace(':', '.'))));

    public void incrCreatedHints(VirtualEndpoint address)
    {
        createdHintCounts.get(address).inc();
    }

    public void incrPastWindow(VirtualEndpoint address)
    {
    ...
    public void log()
    {
        for (Entry<VirtualEndpoint, DifferencingCounter> entry : notStored.asMap().entrySet())
        {
    ...
    public static class DifferencingCounter
    {
        ...
        public DifferencingCounter(VirtualEndpoint address)
        {
