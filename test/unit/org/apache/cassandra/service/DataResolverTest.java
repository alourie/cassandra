...
public class DataResolverTest
{
    ...
    private VirtualEndpoint peer()
    {
        try
        {
            return VirtualEndpoint.getByAddress(new byte[]{ 127, 0, 0, (byte) addressSuffix++ });
        }
    ...
    private MessageOut<Mutation> getSentMessage(VirtualEndpoint target)
    {
    ...
    public MessageIn<ReadResponse> readResponseMessage(VirtualEndpoint from, UnfilteredPartitionIterator partitionIterator)
    {
        return readResponseMessage(from, partitionIterator, command);

    }
    public MessageIn<ReadResponse> readResponseMessage(VirtualEndpoint from, UnfilteredPartitionIterator partitionIterator, ReadCommand cmd)
    {
    ...
    {
        Map<VirtualEndpoint, MessageOut> sent = new HashMap<>();
