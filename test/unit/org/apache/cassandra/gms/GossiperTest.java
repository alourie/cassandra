...
public class GossiperTest
{
    ...
    public void testLargeGenerationJump() throws UnknownHostException, InterruptedException
    {
        ...
        VirtualEndpoint remoteHostAddress = hosts.get(1);
    ...
    public void testReloadSeeds() throws UnknownHostException
    {
        ...
        VirtualEndpoint addr = VirtualEndpoint.getByAddress(InetAddress.getByName("127.99.1.1"));
        int nextSize = 4;
        List<VirtualEndpoint> nextSeeds = new ArrayList<>(nextSize);
        for (int i = 0; i < nextSize; i ++)
        {
            gossiper.seeds.add(addr);
            nextSeeds.add(addr);
            addr = VirtualEndpoint.getByAddress(InetAddresses.increment(addr.address));
        }
        ...
        for (VirtualEndpoint a : nextSeeds)
        ...
        for (VirtualEndpoint a : gossiper.seeds)
        {
            Assert.assertTrue(loadedList.contains(a.toString()));
        ...
        addr = VirtualEndpoint.getByAddress(InetAddress.getByName("127.99.2.1"));
        int disjointSize = 3;
        List<VirtualEndpoint> disjointSeeds = new ArrayList<>(disjointSize);
        ...
        for (VirtualEndpoint a : disjointSeeds)
        {
            Assert.assertTrue(gossiper.getSeeds().contains(a.toString()));
            Assert.assertTrue(loadedList.contains(a.toString()));
        }
        ...
        DatabaseDescriptor.setSeedProvider(new TestSeedProvider(new ArrayList<VirtualEndpoint>()));
        loadedList = gossiper.reloadSeeds();
    ...
    {
        private List<VirtualEndpoint> seeds;

        TestSeedProvider(List<VirtualEndpoint> seeds)
        {
            this.seeds = seeds;
        }
