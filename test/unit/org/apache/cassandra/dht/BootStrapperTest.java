...
public class BootStrapperTest
{
    ...
    private RangeStreamer testSourceTargetComputation(String keyspaceName, int numOldNodes, int replicationFactor) throws UnknownHostException
    {
        ...
        VirtualEndpoint myEndpoint = VirtualEndpoint.getByName("127.0.0.1");
        ...
        {
            public boolean isAlive(VirtualEndpoint ep)
            {
                return true;
            }

            public void interpret(VirtualEndpoint ep) { throw new UnsupportedOperationException(); }
            public void report(VirtualEndpoint ep) { throw new UnsupportedOperationException(); }
            public void registerFailureDetectionEventListener(IFailureDetectionEventListener listener) { throw new UnsupportedOperationException(); }
            public void unregisterFailureDetectionEventListener(IFailureDetectionEventListener listener) { throw new UnsupportedOperationException(); }
            public void remove(VirtualEndpoint ep) { throw new UnsupportedOperationException(); }
            public void forceConviction(VirtualEndpoint ep) { throw new UnsupportedOperationException(); }
        };
        s.addSourceFilter(new RangeStreamer.FailureDetectorSourceFilter(mockFailureDetector));
        s.addRanges(keyspaceName, Keyspace.open(keyspaceName).getReplicationStrategy().getPendingAddressRanges(tmd, myToken, myEndpoint));

        Collection<Map.Entry<VirtualEndpoint, Collection<Range<Token>>>> toFetch = s.toFetch().get(keyspaceName);

        // Check we get get RF new ranges in total
        Set<Range<Token>> ranges = new HashSet<>();
        for (Map.Entry<VirtualEndpoint, Collection<Range<Token>>> e : toFetch)
    ...
    private void generateFakeEndpoints(TokenMetadata tmd, int numOldNodes, int numVNodes, String dc, String rack) throws UnknownHostException
    {
        ...
        for (int i = 1; i <= numOldNodes; i++)
        {
            ...
            VirtualEndpoint addr = VirtualEndpoint.getByName("127." + dc + "." + rack + "." + (i + 1));
    ...
    public void testAllocateTokens() throws UnknownHostException
    {
        ...
        VirtualEndpoint addr = FBUtilities.getBroadcastAddressAndPort();
    ...
    public void testAllocateTokensNetworkStrategy(int rackCount, int replicas) throws UnknownHostException
    {
        ...
        try
        {
            ...
            metadata.updateHostId(UUID.randomUUID(), VirtualEndpoint.getByName("127.1.0.99"));
            metadata.updateHostId(UUID.randomUUID(), VirtualEndpoint.getByName("127.15.0.99"));
            ...
            VirtualEndpoint addr = VirtualEndpoint.getByName("127." + dc + ".0.99");
            allocateTokensForNode(vn, ks, tm, addr);
    ...
    private void allocateTokensForNode(int vn, String ks, TokenMetadata tm, VirtualEndpoint addr)
    {
    ...
    public void testAllocateTokensMultipleKeyspaces() throws UnknownHostException
    {
        ...
        VirtualEndpoint dcaddr = FBUtilities.getBroadcastAddressAndPort();
        ...
        for (int i=11; i<=20; ++i)
        {
            allocateTokensForNode(vn, cks, tm, VirtualEndpoint.getByName("127.0.0." + (i + 1)));
